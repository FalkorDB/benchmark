...
if [[ "${RUN_FALKOR}" == "1" ]]; then
  ...
  cargo run --release --bin benchmark -- load --vendor falkor --size small --endpoint "$FALKOR_ENDPOINT" -b "$BATCH_SIZE" --query-profile "$QUERY_PROFILE"
fi
if [[ "${RUN_FALKOR_2}" == "1" ]]; then
  echo "==> Loading small dataset into FalkorDB (secondary)"
  cargo run --release --bin benchmark -- load --vendor falkor --size small --endpoint "$FALKOR_ENDPOINT_2" -b "$BATCH_SIZE" --query-profile "$QUERY_PROFILE"
fi
if [[ "${RUN_NEO4J}" == "1" ]]; then
  echo "==> Loading small dataset into Neo4j"
  cargo run --release --bin benchmark -- load --vendor neo4j --size small --endpoint "$NEO4J_ENDPOINT" -b "$BATCH_SIZE" --query-profile "$QUERY_PROFILE"
fi
if [[ "${RUN_MEMGRAPH}" == "1" ]]; then
  echo "==> Loading small dataset into Memgraph"
  # --force clears the external Memgraph instance before loading
  cargo run --release --bin benchmark -- load --vendor memgraph --size small --endpoint "$MEMGRAPH_ENDPOINT" -b "$BATCH_SIZE" --force --query-profile "$QUERY_PROFILE"
fi

echo "==> Generating vendor-specific query files (base=${QUERIES_FILE_BASE}, dataset=small, count=${QUERIES_COUNT}, write_ratio=${WRITE_RATIO}, profile=${QUERY_PROFILE})"
...
if [[ "${RUN_FALKOR}" == "1" || "${RUN_FALKOR_2}" == "1" ]]; then
  cargo run --release --bin benchmark -- generate-queries --vendor falkor   --dataset small --size "$QUERIES_COUNT" --name "$FALKOR_QUERIES_FILE"   --write-ratio "$WRITE_RATIO" --query-profile "$QUERY_PROFILE" "${ALGO_QUERY_ARGS[@]}"
fi
if [[ "${RUN_NEO4J}" == "1" ]]; then
  cargo run --release --bin benchmark -- generate-queries --vendor neo4j   --dataset small --size "$QUERIES_COUNT" --name "$NEO4J_QUERIES_FILE"   --write-ratio "$WRITE_RATIO" --query-profile "$QUERY_PROFILE" "${ALGO_QUERY_ARGS[@]}"
fi
if [[ "${RUN_MEMGRAPH}" == "1" ]]; then
  cargo run --release --bin benchmark -- generate-queries --vendor memgraph --dataset small --size "$QUERIES_COUNT" --name "$MEMGRAPH_QUERIES_FILE" --write-ratio "$WRITE_RATIO" --query-profile "$QUERY_PROFILE" "${ALGO_QUERY_ARGS[@]}"
fi
