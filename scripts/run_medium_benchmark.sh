...
cargo run --release --bin benchmark -- load --vendor falkor --size medium --endpoint "$FALKOR_ENDPOINT" -b "$BATCH_SIZE"
...
echo "==> Generating queries file: ${QUERIES_FILE} (dataset=medium, count=${QUERIES_COUNT}, write_ratio=${WRITE_RATIO})"
# Always regenerate so the file contains the latest query catalog + stable q_id fields.
cargo run --release --bin benchmark -- generate-queries --dataset medium --size "$QUERIES_COUNT" --name "$QUERIES_FILE" --write-ratio "$WRITE_RATIO"
