...
| `just synthetic-verify` | **CI non-divergence gate** (the `Synthetic non-divergence` job): record all read ops (`--op all`, medium dataset), `run --recording` **twice** against the same throwaway FalkorDB across the full concurrency sweep + both cache modes, and fail if `report --diff` finds a different `workload_hash` or any per-op result digest. Latency not asserted. |
