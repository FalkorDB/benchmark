...
impl FalkorClientProvider {
    ...
    pub(crate) async fn get_async_connection(&mut self) -> FalkorResult<FalkorAsyncConnection> {
        // Redis async clients default to a 500ms response timeout. Graph algorithm queries
        // can legitimately run for much longer, so we disable that client-side timeout and
        // rely on higher-level query/deadline controls from callers.
    ...
    pub(crate) async fn get_async_replica_connection(
        ...
        // Keep read-only replica paths aligned with the primary async timeout behavior.
        let config = redis::AsyncConnectionConfig::new().set_response_timeout(None);
        match self {
            FalkorClientProvider::Redis {
                sentinel_replica: Some(replica),
        ...
        let manager = match max_inflight {
            Some(limit) => {
                let config = redis::aio::ConnectionManagerConfig::new()
                    ...
                    .set_concurrency_limit(limit.get());
